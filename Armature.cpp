...
map<string, vector<string>>	armature_bone_dependencies = {
	...
	{"spine_l"		, {"spine"}},
	{"spine_r"		, {"spine"}},
	{"leg_up_l"		, {"spine", "spine_l"}},
	{"leg_up_r"		, {"spine", "spine_r"}},
	{"leg_lo_l"		, {"spine", "spine_l", "leg_up_l"}},
	{"leg_lo_r"		, {"spine", "spine_r", "leg_up_r"}},
	{"foot_l"		, {"spine", "spine_l", "leg_up_l", "leg_lo_l"}},
	{"foot_r"		, {"spine", "spine_r", "leg_up_r", "leg_lo_r"}},
	{"thorax"		, {"spine"}},
	{"shoulder_l"	, {"spine", "thorax"}},
	{"shoulder_r"	, {"spine", "thorax"}},
	{"arm_up_l"		, {"spine", "thorax", "shoulder_l"}},
	{"arm_up_r"		, {"spine", "thorax", "shoulder_r"}},
	{"arm_lo_l"		, {"spine", "thorax", "arm_up_l"}},
	{"arm_lo_r"		, {"spine", "thorax", "arm_up_r"}},
	{"head"			, {"spine", "thorax"}}
};
*/
...
void armature_base_init(struct armature* a, float head_len, float spine_len, float thorax_len, float shoulder_radius, float arm_up_len, float arm_lo_len, float spine_radius, float leg_up_len, float leg_lo_len, float foot_len) {
	...
	a->foot_l.ball_coords = { 0.0f, 0.0f };
	...
	a->leg_lo_l.ball_coords = { 0.0f, 0.0f };
	...
	a->leg_up_l.ball_coords = { 0.0f, 0.0f };
	...
	a->spine_l.ball_coords = { 0.0f, 0.0f };
	...
	a->foot_r.ball_coords = { 0.0f, 0.0f };
	...
	a->leg_lo_r.ball_coords = { 0.0f, 0.0f };
	...
	a->leg_up_r.ball_coords = { 0.0f, 0.0f };
	...
	a->spine_r.ball_coords = { 0.0f, 0.0f };
	...
	a->thorax.ball_coords = { 0.0f, 0.0f };
	...
	a->shoulder_l.ball_coords = { 0.0f, 0.0f };
	...
	a->arm_up_l.ball_coords = { 0.0f, 0.0f };
	...
	a->arm_lo_l.ball_coords = { 0.0f, 0.0f };
	...
	a->shoulder_r.ball_coords = { 0.0f, 0.0f };
	...
	a->arm_up_r.ball_coords = { 0.0f, 0.0f };
	...
	a->arm_lo_r.ball_coords = { 0.0f, 0.0f };
...
float armature_fitness(struct armature *base, struct armature *onto_proj) {
	float err = 0.0f;
...
void armature_set_random_genetic(struct armature *a) {
...
void armature_fit(struct armature* base, struct armature* onto_proj) {
	...
	float					fitness_best = 0.0f;

	int population_size = 2048;
	float mutation_rate = 0.05;
	int generations = 1000;

	for (int i = 0; i < population_size; i++) {
		struct armature current = *base;
		armature_set_random_genetic(&current);
		armature_population.push_back(current);
		armature_fitness_f.push_back(armature_fitness(&current, onto_proj));
	...
	for (int g = 0; g < generations; g++) {
		...
		while (armature_population_new.size() < population_size) {
			int i = floor(rand() / (float)RAND_MAX * (population_size - 1));
				...
				if (rand() / (float)RAND_MAX > armature_fitness_f[i] / fitness_max - (fitness_min/fitness_max)) {
					...
					armature_population_new.push_back(armature_combine(&armature_population[i], mate, mutation_rate));
		...
		for (int i = 0; i < population_size; i++) {
			armature_fitness_f.push_back(armature_fitness(&armature_population[i], onto_proj));
			if (fitness_max < armature_fitness_f[i]) fitness_max = armature_fitness_f[i];
			if (armature_fitness_f[i] < fitness_min) {
				fitness_min = armature_fitness_f[i];
				fitness_min_idx = i;
		...
		printf("fitness_max: %f, fitness_min: %f\n", fitness_max, fitness_min);
	}
	printf("scale: %f\n", armature_best.scale);
	for (int i = 0; i < armature_bones.size(); i++) {
	...
	armature_print(&armature_best, true);
	printf("\n"); printf("\n"); printf("\n"); printf("\n"); printf("\n"); printf("\n"); printf("\n");
	armature_print(onto_proj, false);
}
...
struct bone armature_bone_get_abs_pos(struct armature* a, string bone_name) {
	...
	struct vector2f rotation = a->spine.ball_coords;
	struct bone* current_bone;
	struct bone tmp;
	struct bone tmp_last;
	tmp_last.ball_coords = a->spine.ball_coords;
	for (int i = 0; i < armature_bone_dependencies[bone_name].size(); i++) {
		...
		if (armature_bone_dependencies[bone_name][i].compare("spine") != 0) {
			...
			rotation.y += ball_coords_d.y + current_bone->ball_coords.y;
		}
